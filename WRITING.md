...
- **category**: A broad grouping. REUSE an existing category if it fits. Only create a new category if none of the existing ones are suitable. The "Getting started" category is special, because it is shown first on the website. Use it for topics a new user would need early on (e.g. initial setup, first bookings, key settings). Don't put every article in it, only ones relevant to someone just starting out.
...
- **order**: A number for sorting within the category (10, 20, 30 and so on, in multiples of 10 so new articles can be inserted between existing ones)
...
- Explain the WHY, not just the how. For example: "Set your base price. This is what customers pay for a standard walk." is better than just "Set your base price".
...
- Don't use em dashes (—) or en dashes (–). They make an article read as machine-written. In a list that explains labels, use a hyphen: "**Price** - what the customer pays." In a sentence, use two sentences, a comma or a colon instead.
